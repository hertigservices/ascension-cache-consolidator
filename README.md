...
| | records |
|---|---:|
| best single submitted `itemcache.wdb` | 42,616 |
| **union of all submissions** | **76,017** |
...
1. **The same file arrives many times.** The same `itemcache.wdb` shows up inside a
   zip, a rar and a tarball from three different people. 668,000 record instances
   across 409 submitted files collapse to 132,000 distinct records — a 5× duplication
   rate.
...
This tool treats those as different problems. Byte-identical records are collapsed to
